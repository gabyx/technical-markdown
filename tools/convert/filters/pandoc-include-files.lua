...
local function update_contents(blocks, shift_by, include_path)
    local update_contents_filter = {
        ...
        Image = function(image)
            if
                (not includes_relative_to_cwd or image.classes:includes("relative-to-current"))
        ...
        CodeBlock = function(cb)
            if
                (not includes_relative_to_cwd or cb.classes:includes("relative-to-current"))
...
function transclude(cb)
    ...
    for line in cb.text:gmatch("[^\n]+") do
        ...
        if (includes_relative_to_cwd and not cb.classes:includes("relative-to-current")) and path.is_relative(line) then
